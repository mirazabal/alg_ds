...
char* search_booyer_moore(size_t len_needle, char needle[len_needle], size_t len_haystack, char haystack[len_haystack])
{
  ...
  while(i < len_haystack){
    if(haystack[i] != needle[j]){
      const int shift1 = len_needle - 1 - d1.arr[ (int)haystack[i]]; // find next occurence of the missed character 
      const int shift2 = d2.shift[j+1]; // Look if the pattern found so far is repeated within the string
      const int shift = max(1, max(shift1, shift2)); 
      i += shift;
      j = len_needle - 1;
    } else {
